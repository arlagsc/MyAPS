...
class ProductLineMappingDAO:
    ...
    def get_by_company(company_code: str) -> List[Dict]:
        with get_db_connection() as conn:
            rows = conn.execute("SELECT * FROM product_line_mapping WHERE company_code = ?", (company_code,)).fetchall()
            return [dict(row) for row in rows]
    ...
    def get_by_material_group(material_group: str, company_code: Optional[str] = None) -> List[Dict]:
        with get_db_connection() as conn:
            if company_code:
                rows = conn.execute("SELECT * FROM product_line_mapping WHERE material_group = ? AND company_code = ?", (material_group, company_code)).fetchall()
            else:
                rows = conn.execute("SELECT * FROM product_line_mapping WHERE material_group = ?", (material_group,)).fetchall()
            return [dict(row) for row in rows]
...
class CapacityStandardsDAO:
    ...
    def get_all() -> List[Dict]:
        with get_db_connection() as conn:
            rows = conn.execute("SELECT * FROM capacity_standards WHERE is_active = 1 ORDER BY line_id, capacity_type").fetchall()
            return [dict(row) for row in rows]
    
    @staticmethod
    def get_by_line(line_id: str) -> List[Dict]:
        with get_db_connection() as conn:
            rows = conn.execute("SELECT * FROM capacity_standards WHERE line_id = ? AND is_active = 1 ORDER BY capacity_type", (line_id,)).fetchall()
            return [dict(row) for row in rows]
    
    @staticmethod
    def get_by_product(line_id: str, product_code: str) -> Optional[Dict]:
        with get_db_connection() as conn:
            row = conn.execute("SELECT * FROM capacity_standards WHERE line_id = ? AND product_code = ? AND is_active = 1", (line_id, product_code)).fetchone()
...
class LineConfigDAO:
    ...
    def get_all() -> List[Dict]:
        with get_db_connection() as conn:
            rows = conn.execute("SELECT * FROM line_config WHERE is_active = 1 ORDER BY line_id").fetchall()
            return [dict(row) for row in rows]
    
    @staticmethod
    def get_by_type(line_type: str) -> List[Dict]:
        with get_db_connection() as conn:
            rows = conn.execute("SELECT * FROM line_config WHERE line_type = ? AND is_active = 1 ORDER BY line_id", (line_type,)).fetchall()
            return [dict(row) for row in rows]
...
class APILogDAO:
    @staticmethod
    def log(api_type: str, method: str, request_data: str, response_data: str, status_code: int, error_message: str, execution_time_ms: int):
        with get_db_connection() as conn:
            conn.execute('''
            ...
            ''', (api_type, method, request_data, response_data, status_code, error_message, execution_time_ms))
    
    @staticmethod
    def get_recent(limit: int = 100) -> List[Dict]:
        with get_db_connection() as conn:
            rows = conn.execute("SELECT * FROM api_logs ORDER BY created_at DESC LIMIT ?", (limit,)).fetchall()
            return [dict(row) for row in rows]


class CalendarDAO:
    WORKSHOP_TYPES = {'SMT': 'SMT车间', 'DIP': 'DIP车间', 'ASSEMBLY': '总装车间', 'WAREHOUSE': '仓库'}
    
    @staticmethod
    def get_workshops() -> List[Dict]:
        with get_db_connection() as conn:
            rows = conn.execute("""
                SELECT workshop_code, workshop_name, COUNT(*) as days_count, SUM(CASE WHEN is_workday = 1 THEN 1 ELSE 0 END) as workdays FROM calendars GROUP BY workshop_code, workshop_name
            """).fetchall()
            return [dict(row) for row in rows]
        ...
        with get_db_connection() as conn:
            for data in data_list:
                try:
                    ...
                    ''', (
                        data.get('workshop_code'), data.get('workshop_name'), data.get('calendar_date'),
                        data.get('is_workday', 1), data.get('shift_type', 'FULL'),
                        data.get('work_start_time', '08:00'), data.get('work_end_time', '20:00'), data.get('notes')
                    ))
                    count += 1
...
def init_extend_data():
    ...
    with get_db_connection() as conn:
        conn.executemany("INSERT OR IGNORE INTO companies (company_code, company_name, description) VALUES (?, ?, ?)", companies)
    
    # 初始化物料组
    material_groups = [
        ('511', '物料组511', 'A类产品'), ('513', '物料组513', 'B类产品'),
        ('514', '物料组514', 'C类产品'), ('515', '物料组515', 'D类产品'),
        ('534', '物料组534', '单箱体'), ('535', '物料组535', '特殊产品'),
        ('523', '物料组523', '双箱体'), ('555', '物料组555', '特殊配置'),
    ]
    with get_db_connection() as conn:
        conn.executemany("INSERT OR IGNORE INTO material_groups (group_code, group_name, description) VALUES (?, ?, ?)", material_groups)
